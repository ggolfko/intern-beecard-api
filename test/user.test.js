...
describe('Method PUT users test', () => {
  ...
  it('update success', () => {
    ...
    doQuery(sql).then((resp) => {
      ...
      expect(resp.message).to.equal('updated success')
    })
  })
...
describe('Method DELETE users test', () => {
  ...
  it('Delete a user by id', () => {
    let sql = `SELECT * FROM users WHERE username LIKE 'babababana' AND email LIKE 'aasdsdbababa@mails.com'`
    setTimeout(function () {
      doQuery(sql).then((resp) => {
        return resp
      }).then((resp) => {
